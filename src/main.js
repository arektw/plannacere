...
document.addEventListener('DOMContentLoaded', function() {
  ...
  const mobileMenuLinks = mobileMenu.querySelectorAll('.nav-link, .cta-primary');
  ...
  const animateElements = document.querySelectorAll('.process-step, .offer-card, .benefit-card, .testimonial-card');
  ...
  if (testimonialsWrapper && testimonialsTrack && prevBtn && nextBtn && pagination) {
    const cards = Array.from(testimonialsTrack.children);
    let currentIndex = 0;

    function updatePagination() {
      pagination.innerHTML = '';
      const cardWidth = cards[0].clientWidth + 40; // width + gap
      const visibleCount = Math.max(1, Math.floor(testimonialsWrapper.clientWidth / cardWidth));
    ...
    function goTo(index) {
      ...
      const maxIndex = Math.max(0, cards.length - Math.max(1, Math.floor(testimonialsWrapper.clientWidth / cardWidth)));
  ...
  function updateActiveNav(activeId) {
    const allNavLinks = [...headerNavLinks, ...mobileNavLinks];
  ...
  // Improved navigation tracking with scroll position
  ...
  // Google Forms iframe handles form submission automatically
});